...
export class FEParamGetTiposCbte {
  private wsfeWSDL: string;

  constructor(wsfeWSDL: string) {
    this.wsfeWSDL = wsfeWSDL;
  }
  async getTiposCbte(cuit: string, token: string, sign: string): Promise<any> {
    ...
    try {
      ...
      const response = await soapClient.FEParamGetTiposCbteAsync({
